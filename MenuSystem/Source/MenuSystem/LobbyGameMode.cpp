...
void ALobbyGameMode::Logout(AController* Exiting)
{
	...
	{
		int32 NumberOfPlayers = GetGameState<AGameStateBase>()->PlayerArray.Num();
		GEngine->AddOnScreenDebugMessage(1, 60.f, FColor::Yellow, FString::Printf(TEXT("Player in game: %d"), NumberOfPlayers-1));

		FString PlayerName = PlayerState->GetPlayerName();
		GEngine->AddOnScreenDebugMessage(-1, 60.f, FColor::Cyan, FString::Printf(TEXT("%s has Exit the Game"), *PlayerName));
	}
}